...
class Users
{
	...
	public function _NewUser($username, $password, $password2, $email, $Question1, $Answer1, $Question2, $Answer2, $recaptcha_challenge_field, $recaptcha_response_field)
	{
		...
		elseif( $this->IsValidName($username) || !$this->NameV($username) || $this->TakenUser($username) )
		{
			$this->status['status']	= "NOK";
	...
	{
		$arrayProhibidos = array('MOD', 'ADM', 'VIP', 'STAFF');
		foreach($arrayProhibidos as $prohi)
		{
	
			if(strstr(strtolower($name), strtolower($prohi)))
			{
				return true;
			}
			return false;
